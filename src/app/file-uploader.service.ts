...
export class FileUploaderService {
  ...
  apiRoot: string = 'http://localhost:64411/api';
  ...
  private _removeFromQueue(queueObj: FileQueueObject) {
    ...
    if (queueObj.status == 1) {

      let url = "api/upload/DeleteFileById?fileId=" + queueObj.fileId
  ...
  //  let url = `${this.apiRoot}/upload/UploadFiles`;
    const req = new HttpRequest('POST', "api/upload/UploadFiles", form, {
      reportProgress: true,
    });
  ...
  private _uploadComplete(queueObj: FileQueueObject, response: HttpResponse<any>) {
    ...
    queueObj.fileId = queueObj.response.body.FileId;
    this._queue.next(this._files);
